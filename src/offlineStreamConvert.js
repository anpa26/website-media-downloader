...
async function downloadAndMuxYoutube(videoUrl, audioUrl, filename, downloadMethod, loadingBar, returnBlob = false) {
    ...
    try {
        ...
        // YouTube processed downloads use MP4 consistently, including
        // multi-audio output. The bundled LibAV variant does not support MKV.
        const targetExt = '.mp4';
...
function normalizeVttForMux(rawText) {
    ...
    while ((match = cuePattern.exec(text))) {
        ...
        if (Number.isFinite(start) && Number.isFinite(end) && end > start && payload) {
            byStart.set(start, { start, end, payload });
        }
    }
    const cues = Array.from(byStart.values()).sort((a, b) => a.start - b.start);
    if (!cues.length) return null;
    for (let i = 0; i < cues.length - 1; i++) {
        if (cues[i].end > cues[i + 1].start) cues[i].end = Math.max(cues[i].start + 0.001, cues[i + 1].start);
    }
    return 'WEBVTT\n\n' + cues.map((cue, index) =>
...
async function embedSubtitlesWithLibAV(videoBlob, subData, containerFormat, filename, downloadMethod, loadingBar, returnBlob = false) {
    ...
    try {
        ...
        // YouTube captions can contain a final cue with an invalid/open-ended
        // duration. Never let that subtitle extend muxing past the media input.
